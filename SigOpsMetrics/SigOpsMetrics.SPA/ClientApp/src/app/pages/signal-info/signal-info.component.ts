...
export class SignalInfoComponent implements OnInit, AfterViewInit {
  ...
  dataSource: MatTableDataSource<SignalInfo> =
    new MatTableDataSource<SignalInfo>();
  ...
  constructor(
    ...
    this.filterSelectList = [
      ...
      },
      {
        ...
        columnProp: "county",
        options: [],
        style: "search-input search-input-county",
      },
      {
        name: "City",
        columnProp: "city",
        options: [],
        style: "search-input search-input-city",
      },
      {
        name: "Priority",
        columnProp: "priority",
        options: [],
        style: "search-input search-input-pr",
      },
      {
        name: "Classification",
        columnProp: "classification",
        options: [],
        style: "search-input search-input-cl",
      },
    ];
    ...
    this.dataSource.filterPredicate = ((data, filter) => {
      const a =
        !filter.signalID ||
        data.signalID.toLowerCase().includes(filter.signalID);
      const b =
        !filter.zoneGroup ||
        data.zoneGroup.toLowerCase().includes(filter.zoneGroup);
      ...
      const e =
        !filter.subcorridor ||
        data.subcorridor.toLowerCase().includes(filter.subcorridor);
      const f =
        !filter.agency || data.agency.toLowerCase().includes(filter.agency);
      const g =
        !filter.mainStreetName ||
        data.mainStreetName.toLowerCase().includes(filter.mainStreetName);
      const h =
        !filter.sideStreetName ||
        data.sideStreetName.toLowerCase().includes(filter.sideStreetName);
      const i =
        !filter.milepost ||
        data.milepost.toLowerCase().includes(filter.milepost);
      const j =
        !filter.asOf ||
        !data.asOf ||
        formatDate(data.asOf, "MM/dd/yyyy", "en-US")
          .toLowerCase()
          .includes(filter.asOf);
      const k =
        !filter.duplicate ||
        data.duplicate.toLowerCase().includes(filter.duplicate);
      const l =
        !filter.include || data.include.toLowerCase().includes(filter.include);
      const m =
        !filter.modified ||
        !data.modified ||
        formatDate(data.modified, "MM/dd/yyyy", "en-US")
          .toLowerCase()
          .includes(filter.modified);
      const n = !filter.note || data.note.toLowerCase().includes(filter.note);
      const o =
        !filter.latitude ||
        data.latitude.toString().toLowerCase().includes(filter.latitude);
      const p =
        !filter.longitude ||
        data.longitude.toString().toLowerCase().includes(filter.longitude);
      const q =
        !filter.county || data.county.toLowerCase().includes(filter.county);
      const r = !filter.city || data.city.toLowerCase().includes(filter.city);
      const s =
        !filter.priority ||
        data.priority.toLowerCase().includes(filter.priority);
      const t =
        !filter.classification ||
        data.classification.toLowerCase().includes(filter.classification);
      return (
        a &&
        b &&
        c &&
    ...
    this.formControl.valueChanges.subscribe((value) => {
      const filter = {
        ...
        county: value.county.trim().toLowerCase(),
        city: value.city.trim().toLowerCase(),
        priority: value.priority.trim().toLowerCase(),
        classification: value.classification.trim().toLowerCase(),
      } as string;
  ...
  sortData(sort: Sort) {
    ...
    this.sortedData = data.sort((a, b) => {
      ...
      switch (sort.active) {
        case "signalID":
          return this.compareNum(a.signalID, b.signalID, isAsc);
        case "zoneGroup":
          return this.compare(a.zoneGroup, b.zoneGroup, isAsc);
        case "zone":
          return this.compare(a.zone, b.zone, isAsc);
        case "corridor":
          return this.compare(a.corridor, b.corridor, isAsc);
        case "subcorridor":
          return this.compare(a.subcorridor, b.subcorridor, isAsc);
        case "agency":
          return this.compare(a.agency, b.agency, isAsc);
        case "mainStreetName":
          return this.compare(a.mainStreetName, b.mainStreetName, isAsc);
        case "sideStreetName":
          return this.compare(a.sideStreetName, b.sideStreetName, isAsc);
        case "milepost":
          return this.compareFloat(a.milepost, b.milepost, isAsc);
        case "asOf":
          return this.compareDate(a.asOf, b.asOf, isAsc);
        case "duplicate":
          return this.compare(a.duplicate, b.duplicate, isAsc);
        case "include":
          return this.compareNum(a.include, b.include, isAsc);
        case "modified":
          return this.compareDate(a.modified, b.modified, isAsc);
        case "note":
        ...
        case "county":
          return this.compare(a.county, b.county, isAsc);
        case "city":
          return this.compare(a.city, b.city, isAsc);
        case "priority":
          return this.compare(a.priority, b.priority, isAsc);
        case "classification":
          return this.compare(a.classification, b.classification, isAsc);
        default:
          return 0;
      }
    });
