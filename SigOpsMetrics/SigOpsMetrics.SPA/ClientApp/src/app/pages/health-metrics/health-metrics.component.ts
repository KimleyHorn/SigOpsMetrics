...
export class HealthMetricsComponent implements OnInit {
  ...
  mtGraphMetrics: Metrics = new Metrics({
    measure: "maint_plot",
    formatDecimals: 1,
    formatType: "percent",
  });
  ...
  mtBar: Graph = {
    ...
    y: "corridor",
    hoverTemplate:
      "<b>%{y}</b>" + "<br>Percent Health: <b>%{x:.1%}</b>" + "<extra></extra>",
  };
  mtLine: Graph = {
    title: "Weekly Trend",
    x: "month",
  ...
  otGraphMetrics: Metrics = new Metrics({
    measure: "ops_plot",
    formatDecimals: 1,
    formatType: "percent",
  });
  ...
  stGraphMetrics: Metrics = new Metrics({
    measure: "safety_plot",
    formatDecimals: 1,
    formatType: "percent",
  });
  ...
  dataSourceMaintenance: MatTableDataSource<HealthMaintenance> =
    new MatTableDataSource<HealthMaintenance>();
  displayedColumnsMaintenance: string[] = [
    "zone_Group",
    "corridor",
    "percent Health",
    "missingData",
    "detection Uptime Score",
    "ped Actuation Uptime Score",
    "comm Uptime Score",
    "cctv Uptime Score",
    "flash Events Score",
    "detection Uptime",
    "ped Actuation Uptime",
    "comm Uptime",
    "cctv Uptime",
    "flash Events",
  ];
  ...
  dataSourceOperations: MatTableDataSource<HealthOperations> =
    new MatTableDataSource<HealthOperations>();
  displayedColumnsOperations: string[] = [
    "zone_Group",
    "corridor",
    "percent Health",
    "missingData",
    "platoon Ratio Score",
    "ped Delay Score",
    "split Failures Score",
    "travel Time Index Score",
    "buffer Index Score",
    "platoon Ratio",
    "ped Delay",
    "split Failures",
  ...
  dataSourceSafety: MatTableDataSource<HealthSafety> =
    new MatTableDataSource<HealthSafety>();
  displayedColumnsSafety: string[] = [
    "zone_Group",
    "corridor",
    "percent Health",
    "missingData",
    "crash Rate Index Score",
    "kabco Crash Severity Index Score",
    "high Speed Index Score",
    "ped Injury Exposure Index Score",
    "crash Rate Index",
    "kabco Crash Severity Index",
    "high Speed Index",
    "ped Injury Exposure Index",
  ];
  ...
  constructor(
    ...
    this.dataSourceMaintenance.filterPredicate = ((data, filter) => {
      const a =
        !filter.zone_Group ||
        data.zone_Group.toLowerCase().includes(filter.zone_Group);
      const b =
        !filter.corridor ||
        data.corridor.toLowerCase().includes(filter.corridor);
      const c =
        !filter.percentHealth ||
        this.evaluateFilter(data, "percent Health", filter.percentHealth, true);
      const d =
        !filter.missingData ||
        this.evaluateFilter(data, "missing Data", filter.missingData, true);
      const e =
        !filter.detectionUptimeScore ||
        this.evaluateFilter(
          data,
          "detection Uptime Score",
          filter.detectionUptimeScore,
      ...
      const f =
        !filter.pedActuationUptimeScore ||
        this.evaluateFilter(
          data,
          "ped Actuation Uptime Score",
          filter.pedActuationUptimeScore,
      ...
      const l =
        !filter.commUptime ||
        this.evaluateFilter(data, "comm Uptime", filter.commUptime, true);
      const m =
        !filter.cctvUptime ||
        this.evaluateFilter(data, "cctv Uptime", filter.cctvUptime, true);
      const n =
        !filter.flashEvents ||
        this.evaluateFilter(data, "flash Events", filter.flashEvents, false);
    ...
    this.formControlMaintenance.valueChanges.subscribe((value) => {
      const filter = {
        ...
        pedActuationUptimeScore: value.pedActuationUptimeScore
          .trim()
          .toLowerCase(),
    ...
    this.dataSourceOperations.filterPredicate = ((data, filter) => {
      ...
      const j =
        !filter.platoonRatio ||
        this.evaluateFilter(data, "platoon Ratio", filter.platoonRatio, false);
      const k =
        !filter.pedDelay ||
        this.evaluateFilter(data, "ped Delay", filter.pedDelay, false);
      const l =
        !filter.splitFailures ||
        this.evaluateFilter(data, "split Failures", filter.splitFailures, true);
      const m =
        !filter.travelTimeIndex ||
        this.evaluateFilter(
          data,
          "travel time Index",
          filter.travelTimeIndex,
      ...
      const n =
        !filter.bufferIndex ||
        this.evaluateFilter(data, "buffer Index", filter.bufferIndex, false);
      return a && b && c && d && e && f && g && h && i && j && k && l && m && n;
    ...
    this.dataSourceSafety.filterPredicate = ((data, filter) => {
      ...
      const c =
        !filter.percentHealth ||
        data.percentHealth.toLowerCase().includes(filter.percentHealth);
      const d =
        !filter.missingData ||
        data.missingData.toLowerCase().includes(filter.missingData);
      const e =
        !filter.crashRateIndexScore ||
        data.crashRateIndexScore
          .toLowerCase()
          .includes(filter.crashRateIndexScore);
      const f =
        !filter.kabcoCrashSeverityIndexScore ||
        data.kabcoCrashSeverityIndexScore
          .toLowerCase()
          .includes(filter.kabcoCrashSeverityIndexScore);
      const g =
        !filter.highSpeedIndexScore ||
        data.highSpeedIndexScore
          .toLowerCase()
          .includes(filter.highSpeedIndexScore);
      const h =
        !filter.pedInjuryExposureIndexScore ||
        data.pedInjuryExposureIndexScore
          .toLowerCase()
          .includes(filter.pedInjuryExposureIndexScore);
      const i =
        !filter.crashRateIndex ||
        data.crashRateIndex.toLowerCase().includes(filter.crashRateIndex);
      const j =
        !filter.kabcoCrashSeverityIndex ||
        !data.kabcoCrashSeverityIndex ||
        data.modified.toLowerCase().includes(filter.kabcoCrashSeverityIndex);
      const k =
        !filter.highSpeedIndex ||
        data.highSpeedIndex.toLowerCase().includes(filter.highSpeedIndex);
      const l =
        !filter.pedInjuryExposureIndex ||
        data.pedInjuryExposureIndex
          .toLowerCase()
          .includes(filter.pedInjuryExposureIndex);
    ...
    this.formControlSafety.valueChanges.subscribe((value) => {
      const filter = {
        ...
        kabcoCrashSeverityIndexScore: value.kabcoCrashSeverityIndexScore
          .trim()
          .toLowerCase(),
  ...
  ngOnInit(): void {
    ...
    var mm = (this.currentDate.getMonth() + 1) as any; // Months start at 0!
    var dd = this.currentDate.getDate() as any;
    ...
    this.startDate = `${startMonth}-${dd}-${startYear}`;
    this.endDate = `${endMonth}-${dd}-${endYear}`;
  }
  ...
  ngAfterViewInit() {
    ...
    maintMetric.start = this.startDate;
    maintMetric.end = this.endDate;
    ...
    opsMetric.start = this.startDate;
    opsMetric.end = this.endDate;
    this.metricsService
      .getMetrics(opsMetric)
      .subscribe((data) => (this.dataSourceOperations.data = data));
    ...
    safetyMetric.start = this.startDate;
    safetyMetric.end = this.endDate;
  ...
  sortData(sort: Sort) {
    if (this.currentTab === "Maintenance") {
      ...
      this.sortedDataMaintenance = data.sort((a, b) => {
        ...
        switch (sort.active) {
          case "zone_Group":
            return this.compare(a.zone_Group, b.zone_Group, isAsc);
          case "corridor":
            return this.compare(a.corridor, b.corridor, isAsc);
          case "percent Health":
            return this.compare(
              a["percent Health"],
              b["percent Health"],
          ...
          case "missing Data":
            return this.compare(a["missing Data"], b["missing Data"], isAsc);
          case "detection Uptime Score":
            return this.compare(
              a["detection Uptime Score"],
              b["detection Uptime Score"],
              isAsc
            );
          case "ped Actuation Uptime Score":
            return this.compare(
              a["ped Actuation Uptime Score"],
              b["ped Actuation Uptime Score"],
              isAsc
            );
          case "comm Uptime Score":
            return this.compare(
              a["comm Uptime Score"],
              b["comm Uptime Score"],
              isAsc
            );
          case "cctv Uptime Score":
            return this.compare(
              a["cctv Uptime Score"],
              b["cctv Uptime Score"],
              isAsc
            );
          case "flash Event Score":
            return this.compare(
              a["flash Events Score"],
              b["flash Events Score"],
              isAsc
            );
          case "detection Uptime":
            return this.compare(
              a["detection Uptime"],
              b["detection Uptime"],
              isAsc
            );
          case "ped Actuation Uptime ":
            return this.compare(
              a["ped Actuation Uptime"],
              b["ped Actuation Uptime"],
              isAsc
            );
          case "comm Uptime":
            return this.compare(a["comm Uptime"], b["comm Uptime"], isAsc);
          case "cctv Uptime":
            return this.compare(a["cctv Uptime"], b["cctv Uptime"], isAsc);
          case "flash Events":
            return this.compare(a["flash Events"], b["flash Events"], isAsc);
          default:
            return 0;
        }
      });
      ...
      this.sortedDataOperations = data2.sort((a, b) => {
        ...
        switch (sort.active) {
          case "zone_Group":
            return this.compare(a.zone_Group, b.zone_Group, isAsc2);
          case "corridor":
            return this.compare(a.corridor, b.corridor, isAsc2);
          case "percent Health":
            return this.compare(
              a["percent Health"],
              b["percent Health"],
          ...
          case "missing Data":
            return this.compare(a["missing Data"], b["missing Data"], isAsc2);
          case "platoon Ratio Score":
            return this.compare(
              a["platoon Ratio Score"],
              b["platoon Ratio Score"],
              isAsc2
            );
          case "ped Delay Score":
            return this.compare(
              a["ped Delay Score"],
              b["ped Delay Score"],
              isAsc2
            );
          case "split Failures Score":
            return this.compare(
              a["split Failures Score"],
              b["split Failures Score"],
              isAsc2
            );
          case "travel Time Index Score":
            return this.compare(
              a["travel Time Index Score"],
              b["travel Time Index Score"],
              isAsc2
            );
          case "buffer Index Score":
            return this.compare(
              a["buffer Index Score"],
              b["buffer Index Score"],
              isAsc2
            );
          case "platoon Ratio":
            return this.compare(a["platoon Ratio"], b["platoon Ratio"], isAsc2);
          case "ped Delay":
            return this.compare(a["ped Delay"], b["ped Delay"], isAsc2);
          case "split Failures":
            return this.compare(
              a["split Failures"],
              b["split Failures"],
          ...
          case "buffer Index":
            return this.compare(a["buffer Index"], b["buffer Index"], isAsc2);
          default:
            return 0;
        }
      });
      ...
      this.sortedDataSafety = data3.sort((a, b) => {
        ...
        switch (sort.active) {
          case "zone_Group":
            return this.compare(a.zone_Group, b.zone_Group, isAsc3);
          case "corridor":
            return this.compare(a.corridor, b.corridor, isAsc3);
          case "percent Health":
            return this.compare(
              a["percent Health"],
              b["percent Health"],
          ...
          case "missing Data":
            return this.compare(a["missing Data"], b["missing Data"], isAsc3);
          case "crash Rate Index Score":
            return this.compare(
              a["crash Rate Index Score"],
              b["crash Rate Index Score"],
              isAsc3
            );
          case "kabco Crash Severity Index Score":
            return this.compare(
              a["kabco Crash Severity Index Score"],
              b["kabco Crash Severity Index Score"],
              isAsc3
            );
          case "high Speed Index Score":
            return this.compare(
              a["high Speed Index Score"],
              b["high Speed Index Score"],
              isAsc3
            );
          case "ped Injury Exposure Index Score":
            return this.compare(
              a["ped Injury Exposure Index Score"],
              b["ped Injury Exposure Score"],
              isAsc3
            );
          case "crash Rate Index":
            return this.compare(
              a["crash Rate Index"],
              b["crash Rate Index"],
              isAsc3
            );
          case "kabco Crash Severity Index":
            return this.compare(
              a["kabco Crash Severity Index"],
              b["kabco Crash Severity Index"],
              isAsc3
            );
          case "high Speed Index ":
            return this.compare(
              a["high Speed Index"],
              b["high Speed Index"],
              isAsc3
            );
          case "ped Injury Exposure Index":
            return this.compare(
              a["ped Injury Exposure Index"],
              b["ped Injury Exposure Index"],
              isAsc3
            );
          default:
            return 0;
  ...
  compareDate(a: Date, b: Date, isAsc: boolean) {
    ...
    var val =
      (new Date(a).valueOf() - new Date(b).valueOf()) * (isAsc ? 1 : -1);
  ...
  compareFloat(a: string, b: string, isAsc: boolean) {
    return (
      (a.length == 0 ? -1 : parseFloat(a) < parseFloat(b) ? -1 : 1) *
      (isAsc ? 1 : -1)
    );
  }
  ...
  tabChanged(tabChangeEvent: MatTabChangeEvent): void {
    ...
    this.titleService.setTitle(
      "SigOpsMetrics - HealthMetrics - " + this.currentTab
    );
    setTimeout(() => {
      switch (this.currentTab) {
        case "Maintenance":
          !this.dataSourceMaintenance.paginator
            ? (this.dataSourceMaintenance.paginator = this.maintenancePaginator)
            : null;
          break;
        case "Operations":
          this.dataSourceOperations.paginator
            ? (this.dataSourceOperations.paginator = this.operationsPaginator)
            : null;
          break;
        case "Safety":
          this.dataSourceSafety.paginator
            ? (this.dataSourceSafety.paginator = this.safetyPaginator)
            : null;
      }
    });
  ...
  setRegionStatus() {
    this.metricsService.averagesForMonth("North", this.startDate).subscribe(
      (res) =>
        (this.northData = res.map(function (x) {
          return Math.round(x * 100 * 1e2) / 1e2;
        }))
    );
    this.metricsService.averagesForMonth("Southeast", this.startDate).subscribe(
      (res) =>
        (this.southEastData = res.map(function (x) {
          return Math.round(x * 100 * 1e2) / 1e2;
        }))
    );
    this.metricsService.averagesForMonth("Southwest", this.startDate).subscribe(
      (res) =>
        (this.southWestData = res.map(function (x) {
          return Math.round(x * 100 * 1e2) / 1e2;
        }))
    );
    this.metricsService
      .averagesForMonth("Central Metro", this.startDate)
      .subscribe(
        (res) =>
          (this.centralMetroData = res.map(function (x) {
            return Math.round(x * 100 * 1e2) / 1e2;
          }))
      );
    this.metricsService
      .averagesForMonth("Western Metro", this.startDate)
      .subscribe(
        (res) =>
          (this.westernMetroData = res.map(function (x) {
            return Math.round(x * 100 * 1e2) / 1e2;
          }))
      );
    this.metricsService
      .averagesForMonth("Eastern Metro", this.startDate)
      .subscribe(
        (res) =>
          (this.easternMetroData = res.map(function (x) {
            return Math.round(x * 100 * 1e2) / 1e2;
          }))
      );
    this.metricsService.averagesForMonth("", this.startDate).subscribe(
      (res) =>
        (this.statewideData = res.map(function (x) {
          return Math.round(x * 100 * 1e2) / 1e2;
        }))
    );
  }
